...
// Package validation provides validation utilities for the domain layer
package validation

import (
	"fmt"

	"github.com/abitofhelp/family-service/core/domain/errors"
)
...
func (r *validationResult) Error() error {
	...
	return errors.NewValidationError(firstError.message, firstError.field, nil)
}
...
func ValidateNotNil(value interface{}, fieldName string) error {
	if value == nil {
		return errors.NewValidationError("value cannot be nil", fieldName, nil)
	}
	return nil
}
...
func ValidateNotEmpty(value string, fieldName string) error {
	if value == "" {
		return errors.NewValidationError("value cannot be empty", fieldName, nil)
	}
	return nil
}
...
func ValidateMinLength(value string, minLength int, fieldName string) error {
	if len(value) < minLength {
		return errors.NewValidationError(fmt.Sprintf("value must have a minimum length of %d", minLength), fieldName, nil)
	}
	return nil
}
...
func ValidateMaxLength(value string, maxLength int, fieldName string) error {
	if len(value) > maxLength {
		return errors.NewValidationError(fmt.Sprintf("value must have a maximum length of %d", maxLength), fieldName, nil)
	}
	return nil
}